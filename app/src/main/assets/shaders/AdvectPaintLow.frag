...
void main()
{
	...
	vec4 imageSample = texture2D(texImageInput, samplePos);
	vec2 actualImageSample = vec2(dot(imageSample.rb, uTexInputMask), dot(imageSample.ga, uTexInputMask));
#else
	...
	actualImageSample.g *= uImageInputParams.z; // zero out the g channel for single palette mode.
	// The b channel gets covered in the second pass, and it in fact doesn't matter what happens with it, since it's not used at all in the color mode shader piece
...
#if defined(IMAGE_INPUT)
	actualImageSample *= uImageInputParams.y;
	colors = mix(actualImageSample, colors, uImageInputParams.x);
#else
	colors += actualInput;
	colors *= uFadeCoeff;
#endif
